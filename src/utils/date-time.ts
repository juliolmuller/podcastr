...
function convertSecondsToTimeString(numSeconds: number): string {
...
function convertDateToFriendlyString(isoDate: string): string {
  return format(parseISO(isoDate), 'd MMM yy', {
    locale: ptBR,
  });
}

export { convertDateToFriendlyString, convertSecondsToTimeString };
